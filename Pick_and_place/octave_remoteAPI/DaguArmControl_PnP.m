...
function DaguArmControl_PnP()
  ...
  disp('Octave remote API simulation started');
    ...
    [returnCode,cube_handle] = simxGetObjectHandle(clientID,'Cuboid',vrep.simx_opmode_blocking);
    for i = 1:5
      [returnCode, position(i)] = simxGetJointPosition(clientID,shaft_handles_list(i),vrep.simx_opmode_streaming);
    endfor
    [returnCode, cube_position] = simxGetObjectPosition(clientID,cube_handle,-1,vrep.simx_opmode_streaming);
    ...
    while(val2)    
      ...
      [returnCode, cube_position] = simxGetObjectPosition(clientID,cube_handle,-1,vrep.simx_opmode_buffer);   
      disp('Current position(x,y,z) of Cuboid in mm:');disp(cube_position*1000);      
